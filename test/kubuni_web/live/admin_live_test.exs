defmodule KubuniWeb.AdminLiveTest do
  ...
  defp admin_fixture(attrs \\ %{}) do
    ...
    {:ok, admin} = Kubuni.Accounts.update_user_role(user, :admin)
    admin
  end
  ...
  describe "course detail" do
    ...
    test "adds a module through the curriculum editor", %{conn: conn} do
      ...
      assert [%{title: "Storytelling"}] = Kubuni.Catalog.list_modules()
    end
    ...
    test "uploads a lecture video and deletes the lecture", %{conn: conn} do
      ...
      [lecture] = Kubuni.Catalog.list_lectures()
      ...
      on_exit(fn ->
        File.rm(Path.join(:code.priv_dir(:kubuni), "static#{lecture.video_asset_id}"))
      end)

      html = render_click(view, "delete_lecture", %{"id" => lecture.id})
      refute html =~ "Opening hook"
      assert Kubuni.Catalog.list_lectures() == []
    end
    ...
    test "reorders modules through the curriculum editor", %{conn: conn} do
      ...
      course = Kubuni.Catalog.get_course_with_outline!(course.id)
