defmodule KubuniWeb.AdminLectureVideoLiveTest do
  ...
  test "creates a direct upload and stores the signed playback ID when ready", %{
    ...
    expect(Kubuni.MediaProviderMock, :create_upload, fn ^lecture, opts ->
      assert opts == []
      {:ok, %{id: "upload-123", url: "https://storage.mux.test/direct-upload"}}
    end)

    expect(Kubuni.MediaProviderMock, :upload_status, fn "upload-123" ->
  ...
  test "rejects non-admin learners", %{conn: _conn} do
    learner = Kubuni.AccountsFixtures.user_fixture()
