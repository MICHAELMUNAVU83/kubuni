defmodule Kubuni.Payments do
    ...
    case get_payment_by_reference(reference) do
      ...
      %Payment{status: :successful} = payment ->
        {:ok,
         %{
           payment: payment,
           enrollment: Repo.get!(Kubuni.Enrollments.Enrollment, payment.enrollment_id)
         }}
  ...
  def subscribe(%User{id: id}), do: Phoenix.PubSub.subscribe(Kubuni.PubSub, "user:#{id}")
  ...
  defp complete_verified_payment(payment, verification) do
    result =
      ...
      |> Multi.run(:enrollment, fn _repo, %{payment: completed_payment} ->
        enrollment = Repo.get!(Kubuni.Enrollments.Enrollment, completed_payment.enrollment_id)
  ...
  defp provider, do: Application.fetch_env!(:kubuni, :payment_provider)
end
